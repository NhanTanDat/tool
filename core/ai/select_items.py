...
def _build_video_items_text(candidates: List[Dict[str, Any]]) -> str:
    """
    Format list candidate video cho LLM đọc:
...
def _build_image_items_text(candidates: List[Dict[str, Any]]) -> str:
    """
    Format list candidate image cho LLM đọc:
    ...
    for idx, c in enumerate(candidates):
        title = c.get("title") or ""
        url = c.get("url") or ""
        source = c.get("source") or c.get("domain") or ""
        context = c.get("context") or c.get("page_url") or ""
        line = (
            f"{idx}. title={title!r}, source={source!r}, url={url!r}, context={context!r}"
        )
        lines.append(line)
    return "\n".join(lines)


def select_images_with_ai(
    keyword: str,
    candidates: List[Dict[str, Any]],
    max_keep: int = 5,
) -> List[Dict[str, Any]]:
    """
    Để LLM quyết định giữ image nào cho 1 keyword.

    candidates: list dict có ít nhất: url, title (optional), source/context (optional).
    Trả về list đã được lọc (giữ nguyên thứ tự gốc).
    """
    if not candidates:
        return []
    ...
    system_prompt = (
        "You are an image curator that only outputs JSON.\n"
        "For a given search keyword and a list of candidate images (URLs), "
        "you decide which images are most relevant, high quality and suitable "
        "for use as B-roll or illustration.\n"
        "You must ONLY return a JSON object, no explanation."
    )

    items_text = _build_image_items_text(candidates)

    user_prompt = f"""
Keyword: {keyword}

Below is a list of candidate images from a web image search.
Each line is one image with its index, title, source/domain, url and optional context:

{items_text}

Task:
- Pick at most {max_keep} images that best match the keyword and are suitable to be used as B-roll/illustration.
- Prefer images that are:
  - Clearly about the keyword.
  - High-quality / high-resolution.
  - Not tiny icons, not heavy watermarks, not memes/random screenshots unless keyword implies that.
- If none are acceptable, return an empty list.
...
{{ "keep_indices": [] }}
    ...
    try:
        ...
        raise RuntimeError(
            f"AI trả JSON không hợp lệ (select_images_with_ai): {e}\nNội dung: {raw[:500]}"
        )
